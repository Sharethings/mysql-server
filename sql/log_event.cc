...
*/
// flyyear 详细描述了MySQL的各种数据类型的二进制表现形式
static size_t
  ...
  for (size_t i= 0; i < td->size(); i ++)
  {
    ...
    if (!only_parse)
      ...
      size= log_event_print_value(is_null? NULL: value,
                                       td->type(i), td->field_metadata(i),
                                       typestr, sizeof(typestr));
    // sayidzhang 这面判断如果size返回值为0，那么就会函数直接返回了，所以上面的函数不能直接返回0
    // 这面如果直接返回0了
    if (!size)
      return 0;
                                          ...
                                          uchar *rows_buff)
{
  ...
  for (uchar *value= m_rows_buf; value < m_rows_end; )
  {
    ...
    size_t length1;
    // 如果函数print_verbose_one_row直接返回0，那么将列长度解析报错
  ...
  if (is_flashback)
  {
    ...
    switch(ptr[EVENT_TYPE_OFFSET]) {
      ...
      case binary_log::UPDATE_ROWS_EVENT:
        ...
        ev= new Update_rows_log_event((const char*) ptr, fb_size, &fd_evt);
        ev->exchange_update_rows(print_event_info, ptr); // 原来exchange_update_rows作用是当我们update一条语句的时候，他把语句进行逆转
        delete ev;
        break;
    }
  }
